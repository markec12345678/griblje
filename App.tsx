  ...
  return (
    <div className="min-h-screen bg-[#050505] text-[#E4E3E0] selection:bg-[#FF2A2A] selection:text-white">
      <header className="fixed top-0 left-0 right-0 z-40 border-b border-white/10 bg-black/55 backdrop-blur-xl">
        <div className="max-w-7xl mx-auto px-5 h-16 flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-7 font-mono text-xs uppercase text-neutral-400">
            <a href="#zbirka" className="hover:text-white">Zbirka</a><a href="#katalog" className="hover:text-white">Katalog</a><a href="#poti" className="hover:text-white">Poti</a><a href="#zemljevid" className="hover:text-white">Zemljevid</a><a href="#cas" className="hover:text-white">Čas</a><a href="#o-muzeju" className="hover:text-white">O muzeju</a>
          </nav>
      ...
      <section id="top" className="relative h-screen min-h-[720px] flex items-center justify-center overflow-hidden">
        <motion.div style={{ y: heroY }} className="absolute inset-0 scale-110 opacity-50"><img src={museumRecords[1].image} alt="Kolpa pri Gribljah" className="w-full h-full object-cover" referrerPolicy="no-referrer" /><div className="absolute inset-0 bg-[radial-gradient(circle_at_center,transparent_0%,#050505_78%)]" /><div className="absolute inset-0 bg-black/45" /></motion.div>
        <div className="relative z-10 text-center px-5 max-w-6xl"><SectionLabel>Digitalni muzej vasi / Bela krajina</SectionLabel><motion.h1 initial={{ opacity: 0, y: 35 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .8 }} className="font-display text-[20vw] md:text-[15vw] leading-[.78] tracking-[-.07em] text-white">GRIBLJE</motion.h1><p className="mt-10 max-w-2xl mx-auto text-lg md:text-2xl leading-relaxed text-neutral-300">Vas kot muzej. Kraj kot zbirka. Ljudje, hiše, Kolpa, predmeti, spomini in zgodovina na enem mestu.</p><div className="mt-8 flex flex-wrap justify-center gap-2 font-mono text-[11px] uppercase"><span className="border border-[#FF2A2A] px-3 py-2">Živa vas</span><span className="border border-white/25 px-3 py-2">Digitalna zbirka</span><span className="border border-white/25 px-3 py-2">Odprta zbirka</span></div></div>
      ...
      <main className="max-w-7xl mx-auto px-5 py-24">
        <section id="o-muzeju" className="grid lg:grid-cols-[1.1fr_.9fr] gap-12 mb-28 items-end"><div><SectionLabel>00 / Ideja</SectionLabel><h2 className="font-display text-5xl md:text-7xl text-white uppercase leading-[.9]">Ne spletna stran o vasi.<br /><span className="text-[#FF2A2A]">Muzej vasi.</span></h2></div><div className="text-neutral-400 text-lg leading-relaxed border-l border-neutral-800 pl-6"><p>Model sledi logiki živih muzejev: prostor ni samo ozadje, ampak del zbirke. Beamish zgodovino predstavlja skozi zgodovinska območja, vsakdanje življenje, zgodbe, demonstracije in aktivno raziskovanje obiskovalcev.</p><p className="mt-5">Griblje lahko naredijo svojo različico: <strong className="text-white">resnična vas kot digitalni muzej</strong>, kjer je vsaka lokacija lahko vstopna točka v zgodbo.</p></div></section>

        <section id="zbirka" className="mb-28"><div className="flex flex-col md:flex-row md:items-end justify-between gap-7 mb-10"><div><SectionLabel>01 / Zbirka</SectionLabel><h2 className="font-display text-5xl md:text-7xl text-white uppercase leading-none">Raziskuj Griblje</h2></div><div className="font-mono text-xs text-neutral-500 flex items-center gap-2"><BookOpen size={16} /> {filteredItems.length} / {museumRecords.length} ZAPISOV</div></div><div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-2 mb-8">{collectionLayers.map((layer) => <button key={layer.id} onClick={() => setCategory(layer.status === 'aktivno' ? layer.label.toUpperCase().replace('KRAJI', 'KRAJ') : 'VSE')} className="border border-neutral-800 bg-[#090909] p-4 text-left hover:border-[#FF2A2A] transition-colors"><div className="font-mono text-[9px] uppercase text-neutral-500 mb-2">{layer.status}</div><div className="font-display text-lg text-white uppercase">{layer.label}</div><div className="mt-2 text-[11px] leading-relaxed text-neutral-500">{layer.description}</div></button>)}</div><div className="bg-[#0a0a0a] border border-neutral-800 p-4 md:p-5 mb-8 sticky top-16 z-20"><div className="relative mb-4"><Search className="absolute left-4 top-1/2 -translate-y-1/2 text-neutral-600" size={19} /><input aria-label="Iskanje po muzejski zbirki" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Išči: Kolpa, cerkev, 1945 ..." className="w-full bg-black border border-neutral-700 px-12 py-4 text-white outline-none focus:border-[#FF2A2A] font-mono text-sm" /></div><div className="flex flex-wrap gap-2 items-center"><Filter size={15} className="text-neutral-600 mr-1" />{categories.map((item) => <button key={item} onClick={() => setCategory(item)} className={`px-3 py-2 border font-mono text-[10px] uppercase transition ${category === item ? 'bg-[#FF2A2A] text-black border-[#FF2A2A]' : 'border-neutral-700 text-neutral-400 hover:border-white hover:text-white'}`}>{item}</button>)}</div></div><div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">{filteredItems.map((item, index) => <motion.article layout key={item.id} initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * .035 }} tabIndex={0} onKeyDown={(e) => e.key === 'Enter' && setSelected(item)} onClick={() => setSelected(item)} className="group border border-neutral-800 bg-[#090909] hover:border-[#FF2A2A] focus:border-[#FF2A2A] outline-none cursor-pointer transition-colors"><div className="aspect-[4/3] overflow-hidden relative bg-neutral-900"><img src={item.image} alt={item.title} loading={index > 2 ? 'lazy' : 'eager'} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" referrerPolicy="no-referrer" /><div className="absolute inset-x-0 bottom-0 h-24 bg-gradient-to-t from-black/80 to-transparent" /><span className="absolute top-3 left-3 bg-black/90 text-[#FF2A2A] px-2 py-1 font-mono text-[10px]">{item.category}</span></div><div className="p-5"><div className="font-mono text-[10px] text-neutral-600 mb-2">{item.period}</div><h3 className="font-display text-3xl text-white uppercase leading-none mb-3">{item.title}</h3><p className="text-neutral-400 text-sm leading-relaxed line-clamp-3">{item.description}</p><div className="mt-5 pt-4 border-t border-neutral-800 flex items-center justify-between text-[#FF2A2A] font-mono text-[10px] uppercase"><span><MapPin size={13} className="inline mr-1" />{item.location}</span><ChevronRight size={17} /></div></div></motion.article>)}</div>{filteredItems.length === 0 && <div className="border border-dashed border-neutral-700 p-12 text-center text-neutral-500 font-mono text-sm">Ni zadetkov. Poskusi drugo iskanje.</div>}</section>

        <section id="katalog" className="mb-28"><div className="flex flex-col md:flex-row md:items-end justify-between gap-7 mb-10"><div><SectionLabel>01A / Katalog</SectionLabel><h2 className="font-display text-5xl md:text-7xl text-white uppercase leading-none">Stanje zbirke</h2></div><div className="font-mono text-xs text-neutral-500">{catalogTotal} ZAPISOV / {catalogEntries.length} SKLOPOV</div></div><div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-2">{catalogEntries.map((entry) => <div key={entry.kind} className={`border p-5 text-left transition ${entry.status === 'aktivno' ? 'border-neutral-700 bg-[#090909] hover:border-[#FF2A2A]' : 'border-neutral-800 bg-[#070707]'}`}><div className="font-mono text-[10px] uppercase text-neutral-500">{entry.status}</div><div className="font-display text-4xl text-white mt-3">{entry.count}</div><div className="font-display text-lg uppercase text-white">{entry.label}</div><p className="mt-2 text-[11px] leading-relaxed text-neutral-500">{entry.description}</p></div>)}</div></section>

        <section id="poti" className="mb-28"><div className="flex flex-col md:flex-row md:items-end justify-between gap-7 mb-10"><div><SectionLabel>01B / Poti</SectionLabel><h2 className="font-display text-5xl md:text-7xl text-white uppercase leading-none">Raziskuj kot obiskovalec</h2></div><div className="font-mono text-xs text-neutral-500">{museumTrails.length} KURIRANE POTI</div></div><p className="max-w-3xl text-neutral-400 text-lg leading-relaxed mb-8">Muzej ni samo seznam zapisov. Izberi pot in odkrivaj obstoječe zgodovinske točke v smiselnem zaporedju.</p><div className="grid grid-cols-1 md:grid-cols-3 gap-4">{museumTrails.map((trail, index) => { const trailRecords = trail.recordIds.map(recordById).filter((record): record is MuseumRecord => Boolean(record)); return <article key={trail.id} className="border border-neutral-800 bg-[#090909] p-6 hover:border-[#FF2A2A] transition-colors"><div className="font-mono text-[10px] text-[#FF2A2A] uppercase">Pot {String(index + 1).padStart(2, '0')}</div><h3 className="font-display text-3xl text-white uppercase leading-none mt-3">{trail.title}</h3><p className="mt-4 text-sm leading-relaxed text-neutral-400">{trail.description}</p><div className="mt-6 space-y-2">{trailRecords.map((record, step) => <button key={record.id} onClick={() => setSelected(record)} className="w-full flex items-center justify-between gap-3 border border-neutral-800 px-3 py-3 text-left hover:border-white transition-colors"><span><span className="block font-mono text-[9px] text-neutral-600 uppercase">{String(step + 1).padStart(2, '0')} · {record.category} · {record.period}</span><span className="block font-display text-lg text-white uppercase mt-1">{record.title}</span></span><ChevronRight size={16} className="shrink-0 text-[#FF2A2A]" /></button>)}</div><div className="mt-4 font-mono text-[9px] uppercase text-neutral-600">{trailRecords.length} muzejski {trailRecords.length === 1 ? 'zapis' : 'zapisi'}</div></article>; })}</div></section>
