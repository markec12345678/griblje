...
export function MuseumSourcePanel({ sourceIds, sourceById }: Props) {
  const sources = sourceIds.map((id) => sourceById.get(id)).filter((source): source is MuseumSource => Boolean(source));
            ...
            <article key={source.id} className="border border-neutral-800 bg-black/30 p-4">
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0 flex-1">
                  <div className="font-mono text-[9px] uppercase text-neutral-500">{statusLabel[source.verificationStatus]} · {source.type}</div>
                  ...
                  {source.url && (
                    <a href={source.url} target="_blank" rel="noreferrer" className="mt-4 inline-flex items-center gap-2 border border-neutral-700 px-3 py-2 font-mono text-[9px] uppercase text-neutral-300 hover:border-white hover:text-white">
